...
public static void main(String[] args) {
	...
	if(result){
	  ...
	  for(int i = 0; i<N; i++) for(int k = 0; k<L; k++){
			...
			else if((sameCategoryBehind[i][k].dom().value() == 1 && lowCatBehind[i][k].dom().value() == 1) || afterBehind[i][k].dom().value() == 1){
				//Move left
				writer.print("<< ");
			}
			else if((sameCategoryNext[i][k].dom().value() == 1 && lowCatNext[i][k].dom().value() == 1) || afterNext[i][k].dom().value() == 1){
				//Move right
				writer.print(">> ");
			}
